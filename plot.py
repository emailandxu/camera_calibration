...
def plotPose(ax, t=None, R=None, scale=None, l_width=2, text=None):
    """
    ...
    if (text is not None):
        ax.text(x_axis[0, 0], x_axis[0, 1], x_axis[0, 2], text)

    return None


def plotImage(ax, img, t=None, R=None, size=np.array((1, 1)), img_scale=8):
    """
    ...
    corners = np.array(([0., 0, 0], [0, 0, size[0]],
                        [size[1], 0, 0], [size[1], 0, size[0]]))
...
if __name__ == "__main__":
    ...
    poses = from_colmap("resources/ambu/output/cubic/sparse/images.txt", "resources/ambu/output/cubic")
    print(len(poses))
    for idx, (path, mat) in enumerate(poses.items()):
        mat = np.array(mat)
        t, R = mat[:3, 3][np.newaxis, :], mat[:3, :3]
        print(path, t) #, R, t.shape, R.shape)
        plotPose(ax, t, R, scale=np.ones(3) * 0.5, l_width=3, text=f"{idx}")
        img = (np.random.rand(64, 48, 3)*255).astype("u1")
        img = imread(path)[::-1].transpose([1, 0, 2])
        plotImage(ax, img, t, R, img_scale=64)
    plt.show()
